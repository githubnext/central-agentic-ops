...
**Update note (2026-09-05 second material refresh):** `activity/github-telemetry.mjs` now attaches a normalized, bounded call-stack (`GITHUB_TELEMETRY_STACK_TRACE_LIMIT`, 24 frames) to each recorded ledger entry via `defaultGithubTelemetryStackTrace`/`normalizeGithubTelemetryStackTrace`, so `cao-gh.jsonl` and the dashboard's GitHub API telemetry view can attribute each `rate_limit` probe to its calling phase. This is additional metadata captured on the same eight already-inventoried per-run `gh api rate_limit` probes; it adds no new API request. `dashboard/local-server.mjs` was substantially reworked (WebSocket Copilot protocol, host allowlisting) but its predownload path (one artifact-list query plus one artifact download before serving) is unchanged. The `self-care-dashboard-performance.md` evidence-artifact path was renamed to a workspace-relative directory; this does not change its GitHub API or `gh aw logs` behavior. No other change since the prior refresh (14 commits, mostly dashboard UI/component refactors) affects collection, predownload, indexing, or caching behavior recorded in this audit.

**Update note (2026-09-05 material refresh):** A new `self-care-dashboard-performance.md` worker (package `SelfCare`) was added, dispatched by `self-care.md` on its existing schedule/topology. It runs Lighthouse/Playwright performance audits against the built dashboard site and persists only a local round-robin rotation state file (`/tmp/gh-aw/cache-memory/dashboard-performance-rotation.json`, capped at 30 recent entries) plus an evidence artifact upload; it issues no `gh aw logs` calls and no direct GitHub API/REST/GraphQL requests in its prefetch or evaluation steps, so it adds no new collection or N+1 acquisition path. No operational-value grader exists yet for this worker. Other changes since the prior refresh (`max-daily-ai-credits: -1` added across CAO workflows including `shared/control.md`; the intermediate CAO workflow import removed so workflows import `shared/control.md` directly; dashboard component/grader-packaging refactors) do not change any GitHub API, `gh aw logs`, predownload, indexing, or caching behavior recorded in this audit.

**Update note (2026-09-04 material refresh):** The `ambient-context` package was consolidated into `AW Optimization`: `ambient-context-agents-md-curator.md` and `ambient-context-skills-curator.md` are now `optimization-agents-md-curator.md` and `optimization-skills-curator.md`, dispatched as workers by `optimization.md` alongside the existing AI-credit auditor/optimizer; the operational-value grader `ambient-context-agents-md-curator-operational-value.sh` was renamed to `optimization-agents-md-curator-operational-value.sh`. This is a path and dispatch-topology rename only; the per-pull-request list/files/review-comment N+1 acquisition pattern and the grader's issue/PR pagination and per-candidate file reads are unchanged in behavior. References below use the current `optimization-*` names. Imported PR automation workflows (`design-decision-gate.md`, `mattpocock-skills-reviewer.md`, `pr-sous-chef.md`) and the `self-care-open-source-failures.md` worker add prefetch and N+1 acquisition paths not previously inventoried; see §3.3 and §5 items 9-10. `.github/workflows/aw-maintenance.md` was earlier renamed to `aw-doctor.md` (package renamed from AW Maintenance to AW Doctor); the `gh aw logs`/API prefetch inventory reflects the current path. No other collection, predownload, indexing, or caching behavior changed since the prior refresh.
