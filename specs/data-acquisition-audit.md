...
| --- | --- | --- |
| `.github/workflows/shared/control.md` | Sparse, shallow `actions/checkout` steps deploy `.github/cao` at `github.workflow_sha` before activation and in the agent job. | Exact-SHA provenance is preserved; each checkout uses `fetch-depth: 1` and cone-mode sparse paths. |
| `.github/cao/control.mjs` | Reads rate limits, policy and target-authority files, repository metadata, workflow inventory pages, safe-output metadata, and target commits. Also checks runner free disk space before repository discovery (no API calls). | `gh api --cache 60s` for helper reads, a capacity check before precompute, bounded repository pages, and clean rate-limit denial. The cache is job-local and short-lived. |
