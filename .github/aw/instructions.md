...
- Install the `activity/` package from the root manifest. It owns the scheduled workflow activity index, local composite action, cache key contract, and index schema.
...
- Restore workflow and run inventory through `.github/actions/cao-activity`; do not recreate activity indexing or cache maintenance in the dashboard builder.
