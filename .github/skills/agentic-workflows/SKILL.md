...
For service-account-triggered worker dispatches, use `on.bots: ["github-actions[bot]"]` in the shared worker/control workflow configuration rather than `on.roles: all` or duplicating the setting across every worker.
