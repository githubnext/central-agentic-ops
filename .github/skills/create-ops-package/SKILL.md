...
- when `safe-outputs.create-issue` is enabled, `labels: [<package-slug>]`; also enable `safe-outputs.add-labels` with `allowed: [<package-slug>]`, `create-if-missing: true`, issue-only scope, the same target repository, and a `max` matching `create-issue`; require the prompt to pair every `create_issue` call with an `add_labels` call referencing its temporary ID so every created issue carries the package label even when the label does not exist yet; preferably use `title-prefix: "[<package-slug>:<worker-slug>] "` to identify the owning operation and worker, and instruct the worker to provide only the unprefixed subject because the safe output adds the configured prefix automatically, without repeating it or adding a semantically equivalent category prefix
