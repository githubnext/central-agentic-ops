...
printf '%s\n' "$definition" | jq -e '
    ...
    and (if .baseline.mode == "baseline-comparable" then
        ...
        .baseline.value == null and .baseline.evidenceCutoff == null
      end)
...
for example_name in targetAttained targetMissed missing malformed; do
    ...
    case $example_name in
        ...
        missing|malformed) [[ $result == null ]] || fail "--metric must return null for $example_name" ;;
    esac
done
...
request=$(jq -cn --arg repository "$repository" --arg workflow "$workflow_name" \
    --arg sha "$adoption_commit" --arg createdAt "$created_at" --arg evidenceAt "$evidence_at" '
    {schemaVersion:1,run:{id:"1",attempt:1,repository:$repository,workflow:$workflow,
      ref:"refs/heads/main",sha:$sha,eventName:"workflow_dispatch",createdAt:$createdAt},
      evidenceAt:$evidenceAt,case:null,event:{},config:{verification:true}}')
...
printf '%s\n' "$grade_run" | jq -e --arg evidenceAt "$evidence_at" '
    ...
    and (.evidenceCutoff | timestamp) and (.maturesAt | timestamp)
