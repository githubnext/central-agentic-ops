...
- Materialize `.github/cao/admit.sh`, `.github/cao/precompute.sh`, and `.github/cao/resolve.mjs` from that same immutable CAO commit. These are control-repository-owned runtime files; gh-aw package resources cannot and must not install them under `.github/aw`.
...
7. Install the root CAO package. `gh aw add-wizard` reads root `aw.yml`, installs its orchestrators, workers, shared controls, skills, and resources, applies its Copilot authentication configuration, and compiles the workflow lock files:
    ...
    mkdir -p .github/cao
    for cao_file in admit.sh precompute.sh resolve.mjs; do
    ...
    chmod +x .github/cao/admit.sh .github/cao/precompute.sh
    ```
...
- `.github/cao/admit.sh`, `.github/cao/precompute.sh`, or `.github/cao/resolve.mjs` cannot be materialized from the selected immutable CAO ref;
