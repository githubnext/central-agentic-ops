...
name: "EU CRA Compliance / Article 14 Reporting Readiness"
max-ai-credits: 150
...
env:
  CENTRAL_AGENTIC_OPS_WORKER_ENABLED: ${{ vars.CENTRAL_AGENTIC_OPS_EU_CRA_COMPLIANCE_ARTICLE_14_REPORTING_READINESS_ENABLED || 'true' }}
  CENTRAL_AGENTIC_OPS_WORKER_MAX_MODE: ${{ vars.CENTRAL_AGENTIC_OPS_EU_CRA_COMPLIANCE_ARTICLE_14_REPORTING_READINESS_MAX_MODE || 'staged' }}
...
run-name: "CRA Article 14 readiness · ${{ inputs.target_repo }} · ${{ inputs.safe_output_mode || (inputs.preview_only == 'true' && 'staged' || 'live') }}"
...
safe-outputs:
  ...
  create-issue:
    expires: 30d
    title-prefix: "[eu-cra:article-14-readiness] "
