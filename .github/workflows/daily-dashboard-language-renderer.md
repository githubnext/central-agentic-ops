...
- Treat the complete `dashboard/report/` implementation as the authoritative non-normative UX reference. Inspect `report.mjs` for GitHub Primer design language, information density, navigation, responsive layout, status and mode treatments, charts, tables, spacing, typography, color, and interaction behavior. Inspect the sibling inventory, deployed-workflow, AI Credit usage, and operational-value modules for the source availability, completeness, coverage-window, provenance, workflow-topology, and maturity states that the report presents. Migrate those features only through specification-governed data contracts, `dashboard.json`, and generic presenter primitives; never copy collection logic or page-specific rendering logic.
...
13. **Parity** — inventory the features and user-visible presentation patterns of every module under `dashboard/report/`, recording report presentation separately from producer-derived availability, completeness, coverage, provenance, topology, usage, and maturity semantics in `PLAN.md`; then express supported features as YAML configuration plus data fixtures and reusable presentation primitives, closing both checklists incrementally.
