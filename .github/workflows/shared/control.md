...
Every human-facing durable worker output must use progressive disclosure. Its first human-readable section must be `### Executive Summary`, giving the decision-relevant result, critical findings, key metrics, and recommended next actions concisely. Keep that section and other critical information visible. Put non-essential background, verbose supporting evidence, logs, and per-item breakdowns inside `<details><summary>...</summary>...</details>` sections. Metadata markers may precede the executive summary when another contract requires them.
