...
run-name: "EU CRA Advisor · ${{ inputs.target_repo || 'auto' }} · ${{ inputs.safe_output_mode || 'mode' }}"
...
env:
  ...
  CENTRAL_AGENTIC_OPS_MODE: ${{ vars.CENTRAL_AGENTIC_OPS_EU_CRA_COMPLIANCE_MODE || 'review' }}
  GH_AW_SAFE_OUTPUT_MODE: ${{ inputs.safe_output_mode || vars.CENTRAL_AGENTIC_OPS_EU_CRA_COMPLIANCE_MODE || 'review' }}
  REVIEW_OUTPUT_REPO: ${{ inputs.safe_output_repo || github.repository }}
  SAFE_OUTPUT_REPO: ${{ (inputs.safe_output_mode || vars.CENTRAL_AGENTIC_OPS_EU_CRA_COMPLIANCE_MODE || 'review') == 'review' && (inputs.safe_output_repo || github.repository) || '' }}
  TARGET_REPO: ${{ inputs.target_repo || '' }}

if: (vars.CENTRAL_AGENTIC_OPS_EU_CRA_COMPLIANCE_ENABLED || 'true') == 'true'
...
imports:
  - uses: shared/control.md
    with:
      ...
      role: orchestrator
      rollout_percent: ${{ inputs.rollout_percent || vars.CENTRAL_AGENTIC_OPS_EU_CRA_COMPLIANCE_ROLLOUT_PERCENT || '100' }}
      max_repos: ${{ inputs.max_repos || vars.CENTRAL_AGENTIC_OPS_EU_CRA_COMPLIANCE_MAX_REPOS || '1' }}
      ...
      monthly_credit_budget: ${{ vars.CENTRAL_AGENTIC_OPS_EU_CRA_COMPLIANCE_MONTHLY_AI_CREDIT_BUDGET || '0' }}
