...
test("Dashboard package supports embedded and explicit standalone deployment", () => {
  ...
  assert.match(buildWorkflow, /Require cached activity logs[\s\S]*?cao-activity\/gh-aw-logs\.json/);
  assert.match(buildWorkflow, /Transform activity logs into the deployed workflow index[\s\S]*?node "\$ACTIVITY_INDEXER"/);
  assert.match(buildWorkflow, /Collect operational-value observations/);
  assert.match(buildWorkflow, /Collect durable dashboard records/);
  assert.doesNotMatch(activityWorkflow, /control-settings\.mjs|inventory\.mjs|operational-values\.mjs|records\.mjs|ACTIVITY_INDEXER/);
  assert.match(buildWorkflow, /control-settings\.mjs[\s\S]*?\.github\/cao\/src\/control\.mjs[\s\S]*?\.github\/workflows\/cao\.json[\s\S]*?"\$RUNNER_TEMP\/cao-dashboard-data\/control-settings\.json"/);
  assert.match(buildWorkflow, /cp -R \.github\/aw\/dashboard\/site\/\. "\$REPORT_OUTPUT\/"/);
  assert.match(buildWorkflow, /configure-site\.mjs[\s\S]*?"\$REPORT_OUTPUT\/index\.html"[\s\S]*?"\$RUNNER_TEMP\/cao-dashboard-data\/control-settings\.json"/);
  assert.match(buildWorkflow, /bundle-dashboards\.mjs[\s\S]*?"\$REPORT_OUTPUT\/dashboard\.json"[\s\S]*?\.github\/aw\/dashboards/);
  assert.match(buildWorkflow, /REPORT_RECORDS: \$\{\{ runner\.temp \}\}\/cao-dashboard-data\/dashboard-records\.json/);
  assert.match(buildWorkflow, /REPORT_DEPLOYED_WORKFLOWS: \$\{\{ runner\.temp \}\}\/cao-dashboard-data\/deployed-workflows\.json/);
  assert.match(buildWorkflow, /Restore prior dashboard data[\s\S]*?restore-keys:[\s\S]*?cao-dashboard-data-/);
  assert.match(buildWorkflow, /Save dashboard data cache[\s\S]*?key: cao-dashboard-data-\$\{\{ github\.run_id \}\}-\$\{\{ github\.run_attempt \}\}/);
  assert.match(buildWorkflow, /Restore recent GitHub API telemetry[\s\S]*?node "\$GITHUB_TELEMETRY" prepare/);
  assert.equal((buildWorkflow.match(/node "\$GITHUB_TELEMETRY" before /g) || []).length, 4);
  assert.equal((buildWorkflow.match(/node "\$GITHUB_TELEMETRY" after /g) || []).length, 4);
  assert.match(buildWorkflow, /Preserve GitHub API telemetry\n\s+if: always\(\)\n\s+continue-on-error: true/);
  assert.match(buildWorkflow, /REPORT_GITHUB_TELEMETRY: \$\{\{ runner\.temp \}\}\/cao-dashboard-data\/cao-gh\.jsonl/);
  assert.match(buildWorkflow, /Upload GitHub API telemetry\n\s+if: always\(\)[\s\S]*?name: cao-gh[\s\S]*?cao-gh\.jsonl/);
  ...
  assert.match(buildWorkflow, /REPORT_AIC_CACHE: \$\{\{ runner\.temp \}\}\/cao-activity\/logs/);
...
test("Activity package owns the shared collected-data cache contract", () => {
  ...
  assert.match(readme, /never dispatch the activity workflow/);
});
...
test("Documentation Pages deploys docs with the packaged dashboard builder", () => {
  ...
  assert.doesNotMatch(workflow, /inputs\.mode|Refresh activity data/);
  assert.match(workflow, /DISPATCH_INPUTS: '\{"request-id":/);
