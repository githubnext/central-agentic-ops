...
- [ ] Specialized run and dispatch catalog chrome beyond the reusable table controls.
  - [x] Convert runtime triage from its dedicated JavaScript signal renderer to a JSON-declared logical source rendered by the shared signal-list element.
  - [x] Convert the dedicated JavaScript dispatch catalog renderer to a JSON-declared table over JSON-classified dispatch rows and shared table, status, link, search, facet, sort, and empty-state primitives.
...
- [x] Dispatch-event identity and package/standalone dispatch attribution.
...
- Replaced the `dispatch-catalog` element in `dashboard.json` with a declarative `dispatches` table definition and moved package-worker, package-orchestrator, and standalone labels into `dispatch-type-classification.json`.
- Added presentation-independent dispatch-row derivation in `runtime-data.js`, removed the dedicated component, and replaced its component tests with logical-source and declarative-render coverage.
- Verification: `npm run typecheck`, `npm run lint`, `npm test`, `npm run validate:corpus`, `npm run build`, and `npm run test:e2e`.
- Next milestone: continue parity work on page-level runtime/dispatch triage chrome without restoring page-specific rendering.
...
### 2026-08-31 (layout-section chrome refactor)
- Re-read `pages/dashboard/PLAN.md` and selected the next bounded presentation-only slice from the parity cleanup queue: the repeated layout-section heading shell in `src/presenter.js`, which still assembled `.layout-section-header`, `.scope-kicker`, heading ids, descriptions, and count summaries inline instead of reusing the existing shared section-heading primitive.
- Extracted `renderLayoutSectionChrome(...)` into `src/components/view-chrome.js`, composed it from the shared `renderSectionHeading(...)` helper in `src/components/ui-primitives.js`, and replaced the duplicated inline header assembly in `src/presenter.js` without changing section ids, classes, visible copy, or count-summary text.
- Added unit coverage in `test/unit/view-chrome.test.js` locking the shared layout-section header contract for kicker text, heading id, description, and count summary while preserving the existing presenter assertions that cover section rendering paths.
- Verified `pages/dashboard/` quality gates in this run: `npm run build`, `npm run typecheck`, `npm run lint`, and `npm test` all pass. Playwright was not rerun for this bounded presentation-only helper extraction because no browser-only behavior changed; the current browser-gate blocker remains tracked under Infrastructure blockers.
- Next candidates in the queue: extract the repeated definitions/observations dual-region composition in `src/presenter.js`; extract the repeated summary-plus-trend section composition in `src/presenter.js`; extract the next shared pure helpers around subject/provenance/count formatting in `src/presenter.js`.
...
### 2026-08-31 (dashboard migration evidence sweep follow-up)
- Verified evidence completeness from `/tmp/gh-aw/agent/migrate-dashboard/manifest.txt` and current inventories on disk for this run: 255 legacy pages / 255 legacy screenshots / 255 legacy snapshots and 21 next pages / 21 next screenshots / 21 next snapshots.
- Re-read `pages/dashboard/PLAN.md`, then reviewed the rotated order captured in `/tmp/gh-aw/agent/migrate-dashboard/legacy-order.txt` and `next-order.txt`: legacy `index`, `insights/aw-maintenance`, `insights/dependabot`, `insights/eu-cra-compliance`, `insights/optimization`, `insights/uk-ai-advisory`, outcome-detail pages, package pages, repository detail/report/insight pages, run views, runtime, security, value, workflow details, `cost`, `coverage`, `dispatches`, `insights/ambient-context`; next `overview`, `repository-detail`, `workflow-runtime`, `outcome-detail`, `packages`, `package-detail`, `workflows`, `runtime`, `runs`, `dispatches`, `experiments`, `graders`, `evals`, `usage`, `engines-models`, `operational-value`, `findings`, `cost`, `security`, `organizations`, `repositories`.
- Compared the evidence-backed legacy package, repository, workflow, outcome, coverage, runtime, dispatch, cost, and security surfaces against the matching prototype pages by user purpose, using screenshots where available and YAML snapshots for structure, headings, controls, links, and table semantics.
- Added no new parity TODOs: this pass reconfirmed the existing backlog items for mode/report/detail tab matrices, outcomes detail/index coverage, report-controls/export chrome, runtime and dispatch triage framing, repository discovery coverage, dispatch attribution, and execution-map / durable-output semantics already cover the observed operator-affecting gaps.
- Recorded intentional or data-only differences without new backlog expansion where the prototype preserved the page purpose but reflected a different retained dataset, including changed counts on security warning lists, repository inventories, and workflow outcome totals.
- Next rotated review order start for a subsequent run: legacy `index.html` / next `overview`.
