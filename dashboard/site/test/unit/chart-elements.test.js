...
describe('chart element helpers', () => {
  ...
  it('DLS-VIEW-005 DLS-VIEW-006 DLS-VIEW-007 renders JSON-selected chart marks through one generic helper', () => {
    ...
    const histogram = renderChartWidget('histogram', [
      { x: '60', y: 60, color: null },
      { x: '120', y: 120, color: null },
      { x: '180', y: 180, color: null }
    ], [], null, 'Runs', { name: 'Seconds', symbol: 's', significant: 1 });
    ...
    expect(histogram.querySelectorAll('.bar-chart-bar')).toHaveLength(3);
    expect(histogram.querySelector('.bar-chart-bar')?.getAttribute('aria-label')).toContain('60 s');
