...
.horizon-help { position: relative; display: inline-flex; }
.horizon-help-trigger { width: 22px; height: 22px; display: grid; place-items: center; padding: 0; border: 0; border-radius: 50%; background: transparent; color: var(--muted); cursor: help; }
.horizon-help-trigger:hover { background: var(--neutral-muted); color: var(--fg); }
.horizon-help-trigger .octicon { width: 14px; height: 14px; }
.horizon-tooltip { width: min(320px, calc(100vw - 28px)); position: absolute; z-index: 20; top: calc(100% + 8px); right: 0; display: grid; gap: 10px; padding: 12px; border: 1px solid var(--border); border-radius: 6px; background: var(--canvas); box-shadow: 0 8px 24px color-mix(in srgb, var(--canvas-inset) 45%, transparent); color: var(--fg); font-weight: 400; line-height: 1.4; white-space: normal; visibility: hidden; opacity: 0; pointer-events: none; transition: opacity 80ms linear, visibility 80ms linear; }
.horizon-help:hover .horizon-tooltip, .horizon-help:focus-within .horizon-tooltip { visibility: visible; opacity: 1; }
.horizon-tooltip-description { color: var(--muted); }
