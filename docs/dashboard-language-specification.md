...
- **DLS-VIEW-032:** A `chart` view **MAY** declare Boolean `table`; `true` **MUST** include the companion data table and `false` **MUST** omit it. When `table` is omitted, line and pie charts **MUST** omit the table and bar charts **MUST** include it. Other marks **MUST NOT** declare `table`.
