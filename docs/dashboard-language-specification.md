...
`navigation-page` identifies the canonical page whose navigation item remains current while the custom page is active. The presenter uses that page as the custom page's parent breadcrumb. This supports detail and diagnostic subpages without requiring presentation components to contain navigation policy.
...
The optional table-column field `display` is `text`, `status`, `grader-status`, `mode`, `active-state`, `label`, or `digest` and defaults to `text`. It selects presentation independently from the field name. Named UI element values include `status-summary`, `meter-list`, `attention-list`, `domain-attention`, `record-cards`, `coverage-diagnostics`, `package-activity`, `package-detail`, `package-reports`, `dispatch-catalog`, `repository-workflows`, `outcome-detail`, and `workflow-topology`; renderers dispatch these values without inferring behavior from page IDs, view IDs, or source contents. The overview domain-attention element keeps its six operational domains distinct while the remaining elements can be independently assembled through `views`, `sections`, and `layout`.
...
- **DLS-VIEW-026:** A custom page `route`, when present, **MUST** be a mapping containing `hash-query-parameter`, `navigation-page`, or both. Each present value **MUST** be a canonical identifier. Built-in pages **MUST NOT** declare `route`.
