...
- **DLS-PAGE-002:** The `overview` page **MUST** expose rollout-mode filtering, a managed-repository summary number, workflow active-state inventory, run status and conclusion counts and trends, repository and workflow rankings, largest AIC spenders, recent linked findings, an operational-value timeline, and provenance and freshness.
