...
- **DLS-VIEW-032:** A `chart` view **MAY** declare Boolean `table`; `true` **MUST** include the companion data table and `false` **MUST** omit it. When `table` is omitted, line, pie, histogram, and swimlane charts **MUST** omit the table and bar charts **MUST** include it. Other marks **MUST NOT** declare `table`.
- **DLS-VIEW-033:** A swimlane presenter **MUST** expose persistent text labels for every lane, readable time labels on its primary axis, and each observation as an individually focusable mark whose accessible name includes its exact timestamp and category. It **MUST NOT** rely on color alone.
