...
A table encoding may declare `actions` to materialize an intent as a `copy-prompt` or `copy-command` icon button. Each action has non-empty `intent`, `presentation`, `icon`, and `label` values, and may use `when` with a source field and scalar `equals` value to limit the action to matching rows. The presenter copies the intent followed by the complete scalar row context; authors do not interpolate row values into the intent.
...
- **DLS-VIEW-029:** Table actions **MUST** use `copy-prompt` or `copy-command` presentation, a canonical icon, and non-empty intent and label. A conditional action field **MUST** be declared by the table source. A presenter **MUST** copy the applicable row's scalar context with the declared intent and **MUST NOT** require author-defined interpolation.
