...
const DARK_MEDIA_RULE = /@media\(prefers-color-scheme:dark\)\{([^{}]+)\{([^{}]+)\}\}/g;

function themedDataUri(svg, prefersDark) {
  const resolvedSvg = svg.replace(DARK_MEDIA_RULE, (_, selector, declarations) => prefersDark ? `${selector}{${declarations}}` : "");
  return `data:image/svg+xml,${encodeURIComponent(resolvedSvg)}`;
}
...
function createAssets(prefersDark) {
  return [
    { id: SPARKLE_ASSET_ID, w: SPARKLE_SIZE, h: SPARKLE_SIZE, u: "", p: themedDataUri(SPARKLE_SVG, prefersDark), e: 1 },
    { id: STATUS_PENDING_ASSET_ID, w: STATUS_SIZE, h: STATUS_SIZE, u: "", p: themedDataUri(STATUS_PENDING_SVG, prefersDark), e: 1 },
    { id: STATUS_SUCCESS_ASSET_ID, w: STATUS_SIZE, h: STATUS_SIZE, u: "", p: themedDataUri(STATUS_SUCCESS_SVG, prefersDark), e: 1 },
    { id: REPORT_UPDATED_ASSET_ID, w: REPORT_WIDTH, h: REPORT_HEIGHT, u: "", p: themedDataUri(REPORT_UPDATED_SVG, prefersDark), e: 1 },
  ];
}
...
export default function createAnimationData(prefersDark = false) {
  return {
    ...
    ddd: 0,
    assets: createAssets(prefersDark),
    layers: createLayers(),
    markers: [],
  };
}
